...
const IDL = JSON.parse(fs.readFileSync(path.join(ROOT, 'packages/txline/idl/txoracle-devnet.json'), 'utf8'));
const raw = fs.readFileSync(path.join(ROOT, 'devnet-keeper.json'), 'utf8');
...
async function run() {
  ...
  const ata = await getOrCreateAssociatedTokenAccount(
    connection, keypair, TXL_MINT, keypair.publicKey,
    false, 'confirmed', { skipPreflight: false },
...
run().catch(e => { console.error('Error:', e.message ?? e); if (e.response) console.error(JSON.stringify(e.response.data)); process.exit(1); });
