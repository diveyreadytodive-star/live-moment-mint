...
### 2026-07-19 — 배포본 실기 검증으로 근본 원인 규명 + 엔드투엔드 최종 패치

**문제 상황 및 목표**

Vercel 배포본에서 실제 민팅을 돌렸더니 나온 asset이 mpl-core가 아니라 **SPL Token 프로그램 소유**(devnet getAccountInfo로 확인)였다. 즉 여전히 온체인 mpl-core NFT가 아니었다. 마감이 임박해 "실시간→포맷 NFT→온체인 한정 민팅"을 확실히 되는 형태로 끝내는 것이 목표.

**구체적 해결과정 (제 sandbox에서 devnet에 직접 검증)**

1. **배포본/온체인 상태를 직접 조회.** `/api/moments`로 보니 민팅했던 moment는 `momentPda: null`(Cabo Verde 경기 id 9~15 등) → 서버가 온체인 경로를 못 타고 폴백. `momentPda` 있는 건 demo-001의 id 1~3뿐. 그 셋의 PDA를 devnet에서 조회 → 모두 살아있고 소유자가 Momento 프로그램(`CL6e7…F7ja`) 확인.
2. **id 1~3도 close_ts가 이미 지나 UI가 "MINTING WINDOW CLOSED".** DB status는 OPEN이지만 시각 만료. → 새 window 필요.
3. **결정적 검증 — `simulateTransaction`으로 배포 프로그램 해부.** 랜덤/펀딩된 payer로 `open_moment_window`와 `mint_moment`를 devnet에 시뮬레이션(sigVerify:false). 결과: `open_moment_window` err:null 성공. **`mint_moment`는 성공하지만 mpl-core를 전혀 호출하지 않음**(compute 3096~3938, `Program CoREENx… invoke` 로그 없음, MintEvent만 emit). → **배포된 .so는 mpl-core CPI가 없는 옛 버전**임이 확정. 소스 lib.rs엔 CPI가 있으나 재배포가 안 된 상태(SETUP_STATUS에도 "mpl-core CPI = Next Steps"로 기록).
4. **또한 배포 프로그램의 `mint_moment`는 계정 2개(moment, minter)로 받음**을 시뮬레이션으로 확인. 앞선 커밋에서 내가 넣은 5계정 버전은 배포본에선 오히려 실패한다.
5. **엔드투엔드 최종 패치 — 재배포 전/후 자동 대응 설계.** 클라이언트/서버/스크립트를 `NEXT_PUBLIC_MPL_CORE_MINT`(스크립트는 `MPL_CORE_MINT`) 하나로 2계정/5계정을 스위치하게 만듦. 재배포 전엔 2계정(진짜 온체인 tx, asset 없음), 재배포 후엔 env만 켜면 5계정(진짜 mpl-core NFT). 코드 교체 없이 전환.
6. **자동화 산출물 작성:**
   - `apps/keeper/src/scripts/open-and-mint.ts` — keeper 없이 새 온체인 window 개설 + 즉시 민팅 + 검증. 2/5계정 자동 스위치.
   - `scripts/redeploy.sh` — 툴체인 체크 → 빌드(anchor build --no-idl / cargo build-sbf 폴백) → 배포(기존 keypair 있으면 업그레이드, 없으면 새 id + 교체지점 안내) → open+mint 검증까지 원클릭.
   - `scripts/demo-open-window.sh` — REPLAY(풀 UI) 또는 single(단건) 모드로 새 민팅 창 개설.
   - `docs/REDEPLOY_GUIDE.md`, `docs/FINAL_GO.md` — 보안 경고·트러블슈팅·데모 실행 순서.
7. **검증.** web `tsc --noEmit` 에러 0, open-and-mint.ts 타입 통과, 셸 스크립트 `bash -n` 통과.

**결과(성과)**

- 근본 원인 확정: "서버 SPL 우회"의 진짜 이유는 **배포 프로그램에 mpl-core CPI가 없어서**였음. 소스·문서·시뮬레이션을 교차 대조해 규명.
- 코드가 재배포 여부와 무관하게 동작(자동 스위치). 지금 당장 "실시간→온체인 window→한정 민팅 tx"(트랙 A) 데모 가능하고, 재배포하면 env 하나로 "진짜 지갑 NFT"(트랙 B)로 승격.
- 재배포를 원클릭 스크립트로 캡슐화(업그레이드/신규 id 케이스 자동 처리).

**한계 / 사용자 실행 필요 (내가 못 하는 부분)**

- **재배포(anchor build + solana program deploy)와 devnet 실기 서명은 사용자 로컬에서만 가능.** 내 sandbox엔 Solana/Rust 툴체인·지갑 서명 자격증명이 없고, 남의 비밀키로 서명하지 않는다. 그래서 "원클릭 스크립트 + 자동 검증"까지가 내 최대치.
- 기존 program id 업그레이드는 원본 program keypair(로컬 gitignored)가 있어야 함. 없으면 새 id 배포 → 스크립트가 교체지점을 안내.

**배운점**

- **`simulateTransaction`은 배포된 프로그램을 리버스 검증하는 가장 강력한 도구.** 지갑·SOL 없이도 err/logs/compute로 "이 배포본이 실제로 무슨 일을 하는지"를 확정할 수 있다. 소스만 보고 "CPI 있으니 되겠지"라고 가정하면 안 되고, 배포본을 직접 찔러봐야 한다.
- **자동 에이전트의 경계를 인정하고 그 경계까지를 자동화하는 게 최선.** 서명·배포·프로세스 실행을 못 하면, 사용자가 명령 하나로 끝낼 수 있는 원클릭 스크립트 + 자동 검증으로 간극을 메운다.
- **비밀키를 대화에 붙여넣는 사고를 설계로 방지.** 스크립트는 키 경로만 CLI에 넘기고, 문서 최상단에 폐기·재발급 경고를 고정.
